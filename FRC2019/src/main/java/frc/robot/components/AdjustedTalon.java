...
public class AdjustedTalon extends TalonSRX {
	...
	public void set(ControlMode mode, double value) {
		if (mode == ControlMode.PercentOutput) {
			...
			// Right now we're checking voltage once per AdjustedTalon, when we technically
			// only need to do it once per iteration for the whole robot
			...
			/*
			 * else if (current < MAX_CURRENT) { newAtten = (SLOPEC * current) + INTERCEPTC;
			 * newAtten = Math.max(newAtten, MAX_ATTENC); rate *= newAtten; //
			 * System.out.println("Atenn" + newAtten); }
			 */
