...
public class DrivePod {
	...
	/**
	 * Drive for a given distance at a given speed. This method will use closed-loop
	 * control on the speed, but only a simple threshold on the distance - it'll
	 * drive until the threshold is passed.
	 * 
	 * @param inchesPerSecond - the target velocity in inches per second
	...
	public void driveForDistanceAtSpeed(double inchesPerSecond, double inches) {
		...
		System.out.println(name + " seeking a rate of " + inchesPerSecond + " inches per second for " + inches
				+ ", sign=" + targetDeltaSign + ".");
	...
	public boolean isOnTarget() {
		...
		if (getControlMode() == ControlMode.Position) {
			return Math.abs(
					getPositionInches() - getTargetPositionInches()) < Constants.DRIVEPOD_ON_TARGET_THRESHOLD_INCHES;
		} else if (getControlMode() == ControlMode.Velocity) {
